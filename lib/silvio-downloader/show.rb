...
module SilvioDownloader
  class Show < JSONable
    ...
    def find_best_link_episode
      find_link("http://thepiratebay.org/search/#{next_episode_name} -(sneak.peek) -(cocain)/0/7/208")
    end

    def find_best_link_seasson
      find_link("http://thepiratebay.org/search/#{next_seasson_name} -(sneak.peek) -(cocain)/0/7/208")
    end
